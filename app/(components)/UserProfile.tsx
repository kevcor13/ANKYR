        ...
        const fetchUserData = async () => {
            try {
                ...
                if (userResponse.data.status === 'success') {
                    ...
                    setIsFollowing(userResponse.data.data.followers.includes(userData?._id || ""));
                }
        ...
        <>
            ...
            <View className="flex-row items-center justify-between mt-4">
                {targetUser?.profileImage && (
                    // Wrap the string URL in an object with the uri key
                    <Image
                ...
                <TouchableOpacity
                    className={`py-2 px-6 rounded-full ${isFollowing ? 'bg-gray-700' : 'bg-blue-600'}`}
    ...
    return (
        <SafeAreaView className="px-6 bg-black flex-1">
            <ScrollView contentContainerStyle={{ paddingBottom: 16 }}>
                ...
                {activeTab === 'WORKOUTS' && (
                    <View className="flex-1 justify-center items-center mt-8">
                        <Text className="text-gray-500 italic">No workouts available.</Text>
                    </View>
                )}
